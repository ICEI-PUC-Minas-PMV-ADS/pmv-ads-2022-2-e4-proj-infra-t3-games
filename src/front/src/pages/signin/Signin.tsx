...
import { Box, Button, Card, CardActions, CardContent, CircularProgress, Divider, TextField, Typography } from '@mui/material';
...
export const Signin = () => {
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (!email?.length || !password?.length) { alert("todos campos devem ser preenchidos!"); return; }
    if(password.length < 6) { alert("senhas deve possuir pelo menos 6 caracteres!"); return; }
    setIsLoading(true);
  ...
  return (
    <Box width='100vw' height='100vh' display='flex' alignItems='center' justifyContent='center'>
      <Card>
        ...
        <Box>
          <ul>
            <li><Link to='/register'> não possui uma conta? cadastre-se </Link></li>
          </ul>
        </Box>
        <CardActions>
