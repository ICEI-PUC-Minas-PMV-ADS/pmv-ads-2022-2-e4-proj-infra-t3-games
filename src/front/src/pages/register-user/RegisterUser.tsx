...
export const RegisterUser = () => {
    ...
    const handleSubmit = useCallback(() => {
        if (!email?.length || !email?.length || !password?.length) { alert("todos campos devem ser preenchidos!"); return; }
        if(password != confirmPassword) { alert("senhas não correspodem!"); return; }
            ...
            <Card>
                ...
                <CardContent>
                    <Box display='flex' flexDirection='column' gap={2} width={350}>
                        ...
                        <ul>
                            <li><Link to='/confirm-code'>cód. de verificação </Link></li>
                        </ul>
                        <TextField
                            fullWidth
                            type='text'
